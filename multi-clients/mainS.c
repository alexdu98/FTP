...
int main(int argc, char * argv[]) {
  ...
  while(1) {
    ...
    if (fd_circuitV == -1) {
      ...
      // Operation de lecture sur nb_clients a proteger
      ret_m_lock = pthread_mutex_lock(&(s_vars.lock));
      if(ret_m_lock != 0)
	perror("mutex lock nb clients lecture pour pth_create ");
      
      // Initialisation des args qui vont etre passes au thread client
      c_args[s_vars.nb_clients - 1].fd_circuitV = fd_circuitV;
      c_args[s_vars.nb_clients - 1].s_vars = &s_vars;

      ret_pth_create = pthread_create(&clients[s_vars.nb_clients - 1], NULL, &thread_client, NULL);

      ret_m_unlock = pthread_mutex_unlock(&(s_vars.lock));
      if(ret_m_unlock != 0)
	perror("mutex unlock nb clients lecture pour pth_create ");
