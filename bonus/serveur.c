...
void* thread_client (void* args) {
  ...
  while(connecte) {
    ...
    // ##############   CMD SEND   #############
    // #########################################
    else if(m_recv.cmd == SEND){

      printf("CMD : SEND (%s) (socket : %d) \n", m_recv.content, my_props->fd_circuitV);
      ...
      if(access(cheminFichier, F_OK) == 0){
        strcpy(m_send.content, m_recv.content);
        strcat(m_send.content, " -> ");
        strcat(m_send.content, "Nom de fichier deja utilise sur le serveur, merci de changer le votre");
        m_send.cmd = ERROR;
      }
      else{
        m_send.cmd = SEND;
      ...
      // Si la commande n'est pas la taille du fichier
      if(m_recv.cmd != SIZE){
        printf("Erreur : cmd %d attendu, cmd %d recu (socket : %d) \n", SIZE, m_recv.cmd, my_props->fd_circuitV);
        continue;
      }

      // Transformation de la chaine en entier
      tailleFichier = atoi(m_recv.content);
      ...
      if((fichier = fopen(cheminFichier, "wb+")) == NULL){
        perror("Erreur fopen ");
        continue;
      }
      ...
      while(offsetFseek < tailleFichier){
        ...
        resRecv = msg_recv(my_props->fd_circuitV, &m_recv, CLIENT);

        // Sinon si on reçoit une commande différente de CONTENT_FILE
        if(m_recv.cmd != CONTENT_FILE){
          printf("Erreur : commande attendu = %d / reçu = %d \n", CONTENT_FILE, m_recv.cmd);
          break;
        }
        ...
        // Nombre de caractères à écrire dans le fichier
        nbCarALire = resRecv - sizeof(m_recv.size) - sizeof(m_recv.cmd);

        // Ecriture dans le fichier
        resFwrite = fwrite(m_recv.content, 1, nbCarALire, fichier);
      ...
      m_send.size = sizeof(m_send.size) + sizeof(m_send.cmd);
      m_send.cmd = ACK_CONTENT_FILE;

      msg_send(my_props->fd_circuitV, &m_send, CLIENT);
